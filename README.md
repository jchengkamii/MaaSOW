...
所有任务加载资源时，由 agent/recognition_fallback.py 将带 ROI 的图像、
颜色、特征、OCR 等搜索统一包装为“原区域优先，未命中再全图搜索”。
原 Pipeline 文件保留原始区域定义；组合识别的内联子条件和节点引用同样覆盖。
全图分支清除 ROI 偏移，保留模板、阈值与文字匹配条件；OCR 启用文字检测。
动作、流程跳转及原有节点名保持不变，点击使用实际命中的位置。

Python 的治疗行搜索也在原区域无结果时改搜完整截图。
已有命中不会额外执行全图搜索。全图兜底不能解决模板外观或比例完全不匹配的问题。
