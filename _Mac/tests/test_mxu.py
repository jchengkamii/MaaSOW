...
class MxuIntegrationTests(unittest.TestCase):
    def test_interface_is_project_interface_v2(self):
        ...
        self.assertEqual("MacOS", interface["controller"][0]["type"])
        self.assertEqual("^九霄仙府$", interface["controller"][0]["macos"]["title_regex"])
        self.assertEqual("PostToPid", interface["controller"][0]["macos"]["input"])
        self.assertEqual("./.venv/bin/python3", interface["agent"]["child_exec"])
    ...
    def test_every_external_case_becomes_an_mxu_task(self):
        ...
        for case, task in zip(cases, generated["task"]):
            ...
            self.assertEqual(case.id in {"auto_pixiu", "auto_ling_er"}, case.auto_stamina)
        ...
        self.assertEqual("40", treatment_input["default"])
