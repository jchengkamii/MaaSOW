...
class FrontendConfigurationTests(unittest.TestCase):
    ...
    def test_mac_window_lookup_uses_native_adapter(self):
        from unittest.mock import patch
        engine = object.__new__(AutomationEngine)
        with patch("agent.core.find_game_window", return_value="game"):
            self.assertEqual("game", engine.find_window("game"))
        with self.assertRaisesRegex(RuntimeError, "手动打开"):
            engine.find_window("wechat_main")
