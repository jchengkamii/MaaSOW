...
const release = (version = '1.1.12') => ({
  ...
  assets: [{ name: `MaaSOW-scripts-macos-universal-v${version}.zip`, size: 1234,
    browser_download_url: `${github}/releases/download/v${version}/MaaSOW-scripts-macos-universal-v${version}.zip` }],
});
...
test('missing, zero size, wrong source, and full packages are rejected', () => {
  for (const mutate of [
    ...
    (r) => { r.assets[0].name = 'MaaSOW-full-macos-universal-v1.1.12.zip'; },
  ]) {
