...
def generate() -> dict:
    ...
    for case in cases:
        ...
        if case.id == "auto_ling_er":
            ...
            for queue in range(1, 5):
                task["pipeline_override"][f"玲儿集结队列{queue}"] = {"enabled": False}
        if task_options:
            ...
            "inputs": [
                {
                    "name": "目标分钟",
                    "label": "分钟",
                    "description": "请输入 1–10000 的整数，默认 40 分钟",
                    "default": "40",
                    "pipeline_type": "int",
    ...
    if any(case.id == "auto_ling_er" for case in cases):
        options["玲儿集结设置"] = {
            "type": "input", "label": "",
            "description": "勾选队列共同完成集结总次数，返回并消失后计数；等级须在游戏可选范围内",
            "inputs": [
                {"name": "集结等级", "label": "集结等级", "default": "1", "pipeline_type": "int",
                 "verify": r"^[1-9]\d?$", "pattern_msg": "请输入 1–99 的整数"},
                {"name": "集结次数", "label": "集结次数", "default": "1", "pipeline_type": "int",
                 "verify": r"^(?:[1-9]\d{0,3}|10000)$", "pattern_msg": "请输入 1–10000 的整数"},
            ],
            "pipeline_override": {"MXU执行外部用例": {"custom_action_param": {
                "case_id": "auto_ling_er", "rally_level": "{集结等级}", "rally_count": "{集结次数}"
            }}},
        }
        options["玲儿集结队列"] = {
            "type": "checkbox", "label": "集结队列",
            "description": "仅勾选队列参与循环；至少选择一队，默认只选 1",
            "default_case": ["1"],
            "cases": [{"name": str(queue), "label": str(queue),
                       "pipeline_override": {f"玲儿集结队列{queue}": {"enabled": True}}}
                      for queue in range(1, 5)],
        }
    if options:
