...
def new_dispatched_row(existing, rows, count):
    ...
    if len(added) != 1 or added[0].status not in (MarchState.ASSEMBLING, MarchState.OUTBOUND, MarchState.FIGHTING):
        return None
    return added[0]
...
class MarchHandle:
    ...
    last_count: int = 0
    panel_avatar: np.ndarray | None = None
...
class March:
    ...
    def slot_busy(self, image, left, top, size):
        ...
        # Require a near-full-height bar; red clothes and selection borders do not count.
        columns = (green | yellow | red).sum(axis=0) >= size * .85
        run = 0
    ...
    def snapshot(self):
        ...
        self.snapshot_reason = ""
        self.snapshot_status_texts = []
        self.snapshot_rows_reliable = False
        header_region = image[ystart:yend, :xend]
        titles = self.templates(header_region, "header", .75)
        if not titles:
            # The HUD is translucent. Map labels behind 小队 can lower the
            # template score even though its text remains clearly readable.
            labels = self.ocr(header_region, ["^小队$"])
            if len(labels) == 1:
                box = labels[0].box
                pad = max(3, round(w * 4 / 720))
                top = max(0, box.y - pad)
                title = Rect(box.x, top, box.w, min(header_region.shape[0] - top, box.h + 2 * pad))
            else:
                title = None
        else:
            title = titles[0].box if len(titles) == 1 else None
        if title is None and not titles:
            ...
            valid = bool(world.hit and not anchors)
            self.snapshot_reason = "大地图无小队列表" if valid else "小队标题未识别或当前不是大地图"
            return valid, [], 0
        if title is None:
            self.snapshot_reason = "小队标题匹配不唯一"
            return False, [], 0
        top = ystart + title.y
        ...
        if len(counts) != 1:
            # A map label can also produce a false-positive header match.
            # Re-anchor to explicit 小队 text before giving up on the count.
            labels = self.ocr(header_region, ["^小队$"])
            if len(labels) == 1:
                box = labels[0].box
                pad = max(3, round(w * 4 / 720))
                y = max(0, box.y - pad)
                title = Rect(box.x, y, box.w, min(header_region.shape[0] - y, box.h + 2 * pad))
                top = ystart + title.y
                for only_rec in (False, True):
                    counts = self.ocr(image[top:top + title.h, int(w * .27):xend],
                                      [r"[0-4][/／][1-4]"], only_rec=only_rec)
                    if len(counts) == 1:
                        break
        count = None
        if len(counts) == 1:
            ...
            if match:
                ...
                if used <= maximum:
                    count = used
        if count is None:
            self.snapshot_reason = "小队占用数识别失败；仍读取已定位队列状态"
        start = top + title.h
        anchors = self.row_anchors(image[start:yend, :xend])
        complete = count is not None and len(anchors) == count
        if count is not None and not complete:
            self.snapshot_reason = f"小队占用数 {count} 与已识别状态行 {len(anchors)} 不符"
        rows = []
        scale = w / 690
        for anchor in anchors:
            ...
            status_crop = image[top - round(6 * scale):top + round(20 * scale),
                                left + round(43 * scale):x - round(5 * scale)]
            text = self.ocr(status_crop, only_rec=True)
            raw_texts = [r.text for r in text]
            statuses = [parse_status(r.text) for r in text]
            status = next((s for s in statuses if s is not None), None)
            if status is None:
                # Long strips pick up map names behind the translucent row, e.g.
                # 集结中·魂]掌门10. Fixed labels fit in the leftmost 80 pixels;
                # retain the full strip above for outbound coordinates.
                short = self.ocr(status_crop[:, :round(80 * scale)], only_rec=True)
                raw_texts.extend(r.text for r in short)
                fixed = [parse_status(r.text) for r in short]
                status = next((s for s in fixed if s in (MarchState.ASSEMBLING, MarchState.FIGHTING,
                                                         MarchState.RETURNING)), None)
            self.snapshot_status_texts.append(raw_texts)
            rows.append(SquadRow(avatar, status))
        # A collapsed list cannot provide the row anchors for a nonzero count.
        self.snapshot_rows_reliable = 0 < len(rows) <= 4
        return complete, rows, count if count is not None else len(rows)
    ...
    def dispatch_from_panel(self, queue=None, *, existing=None, timeout=15):
        ...
        handle = MarchHandle(desired, selected_avatar, MarchTracker(), panel_avatar=selected_avatar.copy())
        deadline = time.monotonic() + timeout
        previous_added = None
