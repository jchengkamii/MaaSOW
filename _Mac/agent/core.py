...
if sys.version_info[:2] != (3, 12):
    raise RuntimeError(
        ...
        rf"请使用：{PROJECT_DIR / '.venv' / 'bin' / 'python3'}"
    )


from maa.controller import MacOSController
from agent.macos_platform import find_game_window, connect_window
...
class AutomationEngine:
    ...
    def __init__(
        ...
        self._controllers: dict[str, tuple[int, MacOSController]] = {}
    ...
    def find_window(self, target: str):
        ...
        if target != "game":
            raise RuntimeError("Mac 版请手动打开微信小游戏，再运行游戏内任务。")
        return find_game_window()

    def connection_status(self) -> dict[str, bool]:
        return {"game": self.find_window("game") is not None}

    def _controller_for(self, target: str) -> MacOSController:
        window = self.find_window(target)
        if window is None:
            raise RuntimeError("没有找到九霄仙府窗口，请先手动打开游戏。")
        ...
        self.log(f"已连接 Mac 窗口：{window.window_name}，id={window.hwnd}")
        return controller
    ...
    def _ensure_miniprogram_panel(self) -> str:
        if self.find_window("game") is not None:
            return "游戏已经打开"
        raise RuntimeError("Mac 版请手动打开微信中的九霄仙府。")
